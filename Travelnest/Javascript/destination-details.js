...
const details_attractions = document.getElementById('details_attractions');
const detail_costs = document.getElementById('detail-costs');
const bg_image = document.querySelector('.image-overlay');
const dest = destinations.find(d =>
  d.location.toLowerCase() === destName.toLowerCase()
);

if (dest) {
  details_location.textContent       = dest.location;
  details_country.textContent     = dest.country;
  details_image.src            = dest.image;
  details_description.textContent = dest.description;
  bg_image.style.backgroundImage= `url(${dest.image})`;


  dest.attractions.forEach((attraction, i) => {
    details_attractions.innerHTML += `<li>${attraction}</li>`;
  });
  
  dest.costs.forEach((cost, i) => {
    detail_costs.innerHTML += `
      <tr>
        <td>${cost.category}</td>
        <td>${cost.budget}</td>
        <td>${cost.mid}</td>
